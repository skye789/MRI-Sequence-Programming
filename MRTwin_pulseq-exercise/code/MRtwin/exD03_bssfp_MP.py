...
D03.2  put a preparation inversion pulse before the whole image readout 用来surrpress物质
        try different inversion times and dirrenet phantoms.
        You can also try a saturatio pulse (90 degree) but then you ned a strng gradient spoiler after that.
"""
#%%
...
phantom[:,:,3] *= 2 # Tweak dB0
phantom[:,:,3] += 100 # Tweak dB0
...
plt.subplot(3,6,14)
plt.imshow(phantom[:,:,1].transpose(), interpolation='none'); plt.xlabel('T1')
plt.subplot(3,6,15)
plt.imshow(phantom[:,:,2].transpose(), interpolation='none'); plt.xlabel('T2')
plt.subplot(3,6,16)
